      ...
      m_time_out_second_send(0)
{
    ...
    if (!m_initialize) {
        ...
        // cooresponding call to WSACleanup. Choose to do this in the
        // constructor and destructor, to make it scoped initialization.
        WSADATA wsaData;
...
{
    {
        ...
        sockaddr_in address{};
        {
            ...
            std::fill(
                std::begin(address.sin_zero), std::end(address.sin_zero), 0);

            // Implement a default value for the remote host name.
            const std::string& ip_address =
                !host.empty() ? host : DefaultAddress;

            const int result =
                inet_pton(AF_INET, ip_address.c_str(), &address.sin_addr);
            if (result <= 0) {
                // -1 indicates that the AF_INET family is not supported or
                // inputs are malformed. 0 value indicates the address was not
                // parseable as an AF_INET address in "ddd.ddd.ddd.ddd" format.
                CLIENT_ERROR(
                    "failed conversion from host string to address structure");
        ...
        // Check for connection failure. We may want to handle "connection
        // refused" on a different path than general function call failures.
        if (result == -1) {
    ...
    // Read the first message from the service. It is a string description of
    // the remote service.
...
bool Client::waitForData(int time_out_second)
{
    if (!isConnected()) {
        CLIENT_ERROR(
            "failed to wait for incoming data, client is not connected");
        return false;
    }

    // A negative value of time_out_second (-1) indicates that we just want to
...
{
    // Note that this does not enforce the connection state. This may return
    // true even if we are not connected.
...
bool Client::readMessage(data_type& data)
{
    ...
    while (buffer.size() < num_bytes) {
        ...
        if (rc <= 0) {
            // This can indicate a graceful disconnection of the TCP socket
            // stream.
            if (!timed_out) {
                close();
            }
    ...
    if (buffer.size() < num_bytes) {
        CLIENT_ERROR("communication protocol error, failed to read full "
                     "message payload");
        close();
        return false;
    }
...
bool Client::writeMessage(const data_type& data)
{
    ...
    while (num_bytes < message_size) {
        const int rc = socketSend(
            &m_buffer[num_bytes], message_size - num_bytes, timed_out);
        if (rc <= 0) {
            break;
        }
...
int Client::socketSend(const char* buf, std::size_t len, bool& timed_out)
{
    ...
    if ((ec == ETIMEDOUT) || (ec == EAGAIN)) {
        ...
        // connection failed because the connected host has failed to respond.

        // We set a time out for some receive calls. Do not throw an exception
        // if we expect to time out.
        timed_out = true;
#if defined(_WIN32)
    } else if (ec == WSAEINTR) {
        // Interrupted function call. A blocking operation was interrupted by a
        // call to WSACancelBlockingCall.

        // An external thread may have called the #close method for this
        // instance. This may be unsafe, but consider it to be a "graceful
        // close" condition.
#endif
    } else {
...
void Client::socketClose()
{
    ...
    // First, disable sends and receives on this socket. Notify the other side
    // of the connection that we are going away.
    if (shutdown(fd, 2) == -1) {
