...
public:
    ...
    */
    ...
    protected:
        ...
        */
        Element(key_type key, data_type data)
            : m_key(key), m_data(std::move(data))
        {
        }
    ...
    private:
        ...
        /**
          ...
          false</tt> then it contains a sample for each of the <tt>N</tt>
          channels.
        ...
        */
        ...
        /**
          Provide direct access to the internal data buffer from client
          programs.
    ...
    */
    ...
    public:
        ...
        */
        ...
        /**
          ...
          quaternion orientation. Specified as a 16 element array in row-major
          order.
        ...
        */
        ...
        /**
          ...
          @return a three element array <tt>{x, y, z}</tt> of linear
          acceleration channels specified in g or zeros if there is no available
          data
        */
    ...
    /**
      ...
      signals in real units. The Sensor service sends a map of <tt>N</tt> data
      elements. Use this class to wrap a single Sensor data element such that we
      can access individual components through a simple API.
    ...
    */
    ...
    public:
        ...
        /**
          Initialize this container identifier with a packed data array in the
          Raw format.
        ...
        */
        ...
        /**
          ...
          @return a three element array <tt>{x, y, z}</tt> of raw gyroscope
...
private:
    /**
      ...
      std::map<Format::key_type, Format::*Element>. Use the IdToValueArray
      method to handle the low level message parsing.
    */
    template <typename Elem, typename Iterator>
    ...
    */
    ...
    static std::vector<Elem> IdToValueArray(Iterator first, Iterator last)
    {
        ...
        while (first != last) {
            ...
            if (static_cast<size_type>(std::distance(first, last)) <
                sizeof(unsigned)) {
            ...
            // Read optional length integer this element. Some of the services
            // send fixed length samples and do not have the length prefix.
            // Unpack the unsigned 32-bit integer into our host system key type.
            ...
            // Assumes that the binary type stored in Elem::data_type matches
            // the Motion Service format. All services are 32-bit float except
            // for Raw which is 16-bit short.
        ...
        // If we did not consume all of the input bytes this is an invalid
        // message. Roll back intermediate results.
        if (first != last) {
            list.clear();
        }
